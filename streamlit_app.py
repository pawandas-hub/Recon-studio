...
def show_kpi_modal(modal_type: str):
    ...
    if modal_search:
        ...
        search_cols = [c for c in ["Reference", "InvoiceId", "RefId_Ref1", "Ref2_Invoice_No", "SO_ID", "CN_Reference", "GRNID", "Business_Unit", "Overall_Status", "Bank_UTR", "Customer_Id", "DB_SAP_ID", "SAP_Offset_Account"] if c in df_show.columns]
        if search_cols:
    ...
    with st.container(border=True):
        ...
        if results_df is not None and not results_df.empty:
            ...
            if search_txt:
                ...
                search_cols = [c for c in ["Reference", "InvoiceId", "RefId_Ref1", "Ref2_Invoice_No", "SO_ID", "CN_Reference", "GRNID", "Business_Unit", "Overall_Status", "Bank_UTR", "Customer_Id", "DB_SAP_ID", "SAP_Offset_Account"] if c in df_view.columns]
                if search_cols:
            ...
            for _, row in df_preview.iterrows():
                ...
                if rtype == "Sales":
                    ref = _first_valid(row.get("SO_ID"), row.get("CN_Reference"), row.get("GRNID"), row.get("InvoiceId"), row.get("RefId_Ref1"), row.get("Ref2_Invoice_No"), row.get("Reference"))
